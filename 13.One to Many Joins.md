...
Many to Many Relationships are still fairly common. The way this works is, we have, for example, two entities, Books and Authors. The way we say it is that a book can have many authors, for example, two authors could write a book together. And those authors can have many books.
...
Foreign Keys are references to another table from within another given table. In our example, the customer_id in the Orders table is referencing the id in the Customers table. 
...
Now, try and define these tables. Let's start with the customers table.

It has a column called id which will be a primary key, auto increment and it will be an integer.
