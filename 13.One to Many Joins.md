...
Now that we have both tables set up, let's try and retireve information from both of them.
...
Now we have foundboy George's id number. 
...
So, it would be helpful if we could join formation from both tables together. 
...
CROSS JOIN will take take every single row from customers and combine it with every single row from orders. Which means we  will get every combination possible. There's no logic really at all.
...
In other words, a _CROSS JOIN_ just takes every bit of information from both tables that we providedand cross multiplies them.
...
We have now joined both tables. Because we used `*`, we are retreiving all columns and rows from each table.
