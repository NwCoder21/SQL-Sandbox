...
Even though David and Blue don't have any orders, they are still displayed because in a LEFT JOIN we include eveythin in the left table. Because they have no values in the orders table, a `NULL` is displayed for them in the orders part of the joined table. 
...
In the `orders` table, we have no orders which we don't have any orders that don't have a customer_id. Every order has a customer_id which means when we join with the right table, every order will have a customer attachd to it as there's a complete overlap.
...
`IFNULL` works by providing it something to evaluate. Everywhere that evaluation returns `NULL`, then we give it a second arguement to replace the `NULL` with. 
...
CURDATE() is a funnction which enters the current date.
...
Its saying by deleting that all of a sudden you you make the foreign key constraint fail because the foreign key constraint says that every `customer_id` inside of `orders` has to be a real `customer_id`, But if we suddenly delete this row, well, this is no longer a real valid `customer_id` that exists in the database.
...
Automatically that deletion cascaded to any row that had that `customer_id` that was just removed. But we may not want to have that cascading behavior. 
...
![image](https://user-images.githubusercontent.com/107522496/212060366-c831d945-73df-440b-946b-0ea4536f348a.png)
