...
In some situations, we may want to allow the value `NULL`. For example, when setting up a a phone book where every row in the user's table has a name, an email, a cell phone, and maybe a home phone number and maybe a fax number. But not everybody has those. So maybe we make cell phone a requirement or home phone a requirement, but we can make the others nullable (i.e. they don't have to be present).
...
When defining a table, we can add on a Primary Key constraint to a column. This is usuallythe ID column. 
...
The issue at the moment is that to manually incremment the primary key by one each time we enter a new entry is time-consuming.
...
In the above example, we can see that withoutus having to specify the `student_id`, the program did it for us automatically, starting from one. 
